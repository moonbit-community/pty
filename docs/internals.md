...
How `@pty.spawn` works on each platform: executable resolution, environment
merging, and process startup.
...
`.exe` is appended when the last path component has no extension
...
Documented so they stay deliberate:
